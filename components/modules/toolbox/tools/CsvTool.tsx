        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-10 no-scrollbar">
                <div className="flex flex-col gap-8 h-full">
                    ...
                    />
                    <div className="flex-1 bg-white border border-workspace-border rounded-2xl overflow-hidden shadow-sm overflow-auto">
                        <table className="w-full text-left border-collapse">
                            <thead className="sticky top-0 bg-workspace-sidebar z-10">
                                <tr>
                                    {Object.keys(csvParsed[0] || {}).map(header => (
                                        <th key={header} className="px-6 py-4 border-b border-workspace-border text-[10px] font-bold uppercase tracking-widest text-workspace-secondary">
                                            {header}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {csvParsed.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-workspace-selection transition-colors">
                                        {Object.values(row).map((val: any, i) => (
                                            <td key={i} className="px-6 py-4 text-sm font-medium text-workspace-text">{val}</td>
                                        ))}
                                    </tr>
                                ))}
                            </tbody>
