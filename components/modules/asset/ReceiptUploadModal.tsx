...
export const ReceiptUploadModal: React.FC<ReceiptUploadModalProps> = ({ isOpen, onClose, onUploadComplete }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={handleClose}>
            <div className="bg-workspace-canvas w-full max-w-4xl max-h-[90vh] rounded-2xl shadow-2xl flex flex-col md:flex-row overflow-hidden border border-workspace-border/50" onClick={e => e.stopPropagation()}>
                {/* Close Button Mobile */}
                ...
                <div className="flex-1 p-6 md:p-8 border-r border-workspace-border/30 flex flex-col gap-6 overflow-y-auto bg-workspace-sidebar/30">
                    <div className="flex items-center justify-between md:hidden">
                        <h2 className="text-lg font-black uppercase tracking-tight">Add Receipt</h2>
                    </div>

                    <div className={`relative w-full aspect-[3/4] border-2 border-dashed ${imagePreview ? 'border-workspace-border' : 'border-workspace-border hover:border-workspace-accent'} rounded-2xl bg-white flex flex-col items-center justify-center transition-all overflow-hidden group shadow-sm`}>
                        {imagePreview ? (
                            <>
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-workspace-accent text-white font-black uppercase tracking-[0.2em] text-[10px] rounded-xl hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2 shadow-lg shadow-workspace-accent/20 active:scale-95"
                    >
                ...
                {/* Right Panel: Form */}
                <div className="flex-1 p-6 md:p-8 overflow-y-auto bg-workspace-canvas relative">
